...
void ObjectDetector::enqueue(const cv::Mat &frame) {
    ...
    // std::cout << width_ << "  " << height_ << std::endl;    // 1920,1080 low2: 1280,720
    // std::cout << input_name_ << std::endl;                  // input_name = "data"
...
void ObjectDetector::ObjectDetector_Create(
    const DetectorConfig& config,
    const InferenceEngine::InferencePlugin& plugin)
 {
    config_ = config;
...
void ObjectDetector::fetchResults() {
    ...
    for (int det_id = 0; det_id < max_detections_count_; ++det_id) {
        ...
        // 这里有修改
        object.rect.x += image_x;
        object.rect.y += image_y;
        // object.rect.x=object.rect.x*image_p_col/1920-30;
        // object.rect.y=object.rect.y*image_p_row/1080+80;
