...
struct DetectorConfig : public CnnConfig {
    ...
    explicit DetectorConfig(){}     // overload function
    
    void DetectorConfig_Create(const std::string& path_to_model,
                            const std::string& path_to_weights){
                                CnnConfig::CnnConfig_Create(path_to_model,path_to_weights);
                            }
...
public:
    ...
    void ObjectDetector_Create(const DetectorConfig& config,
                            const InferenceEngine::InferencePlugin& plugin);
