...
public:
    ...
    ///
    virtual cv::Size size() const = 0;   // 纯虚函数 包含纯虚函数的类为抽象类 不能生成对象 子类必须实现纯虚函数
...
public:
    ...
    /// \return Descriptor size.
    ///
    virtual cv::Size size() const {    // cv::Size 基本数据结构（width,height）
        return cv::Size(1, handler.size()); // handler.size():longth of result;result_size_*=dim[i]
    }
    ...
    virtual void Compute(const cv::Mat &mat, cv::Mat *descr) {
        handler.Compute(mat, descr);      // VectorCNN::Computer((const std::vector<cv::Mat>& images, std::vector<cv::Mat>* vectors,
                                          // cv::Size outp_shape))
    }
